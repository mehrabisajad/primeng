...
import { ObjectUtils, UniqueComponentId, ZIndexUtils, JDate } from 'primeng/utils';
...
export class Calendar implements OnInit, OnDestroy, ControlValueAccessor {
    ...
    isSelectable(day: any, month: any, year: any, otherMonth: any): boolean {
        ...
        if (this.minDate) {
            ...
            if ((this.isJalali ? jMinDate.getYear() : this.minDate.getFullYear()) > year) {
                validMin = false;
            } else if ((this.isJalali ? jMinDate.getYear() : this.minDate.getFullYear()) === year && this.currentView != 'year') {
    ...
    isValidSelection(value: any): boolean {
        if (this.isSingleSelection()) {
            let jValue;
            if (this.isJalali) {
                jValue = new JDate(value);
            }
            return this.isJalali
                ? this.isSelectable(jValue.getDay(), jValue.getMonth(), jValue.getYear(), false)
        ...
        let isValid = value.every((v: any) => {
            ...
            return this.isJalali
                ? this.isSelectable(jv.getDay(), jv.getMonth(), jv.getYear(), false)
